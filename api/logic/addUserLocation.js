...
const getOrCreateLocation = async (location, locationData) => {
    ...
    .catch((error) => {
        throw new Errors.SystemError('Unexpected error while creating user location:', error)
    })
} 
                    ...
                    return getOrCreateLocation(location, locationData)
                        .then((newLocation) => {
                            ...
                            if (isCurrentLocation) {
                                ...
                                return user.save()
                                .catch((error) => {
                                    throw new Errors.SystemError('Unexpected error while saving user current location:', error)
                                })
                            }
